    ...
    <div className="min-h-screen bg-[#050505] text-white selection:bg-primary/30 selection:text-white overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-2xl border-b border-white/5 bg-black/40">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          ...
          <div className="flex items-center gap-8">
            ...
            <Link href="/register" className="px-6 py-3 rounded-xl bg-primary text-white text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all shadow-xl shadow-primary/20">Scale Now</Link>
          </div>
        </div>
      </nav>
      ...
      <header className="relative pt-48 pb-24 px-6 z-10">
        <div className="max-w-7xl mx-auto text-center space-y-12">
          ...
          >
            <Rocket className="w-4 h-4" /> The Future of Creative Output
          </motion.div>
          ...
          <div className="space-y-6">
            ...
            >
              Scale your <span className="text-primary">Agency</span> <br /> <span className="italic text-white/20">without</span> the overhead.
            </motion.h1>
            ...
            >
              Stop wasting billable hours on scope documents and pitch decks. Automate your creative strategy with Antigravity's Agency Hub.
            </motion.p>
          </div>
          ...
          >
            <Link href="/register" className="group relative inline-flex items-center gap-4 px-12 py-6 rounded-2xl bg-white text-black font-black uppercase tracking-widest text-sm shadow-2xl hover:bg-primary hover:text-white transition-all duration-500 overflow-hidden">
              <span className="relative z-10">Start Scaling for Free</span>
        ...
        <div className="max-w-7xl mx-auto space-y-24">
          <div className="flex flex-col md:flex-row justify-between items-end gap-8">
            <div className="space-y-4">
              <h2 className="text-xs font-black text-primary uppercase tracking-[0.4em]">The Agency Toolkit</h2>
              <p className="text-4xl md:text-6xl font-black tracking-tight uppercase">High-Performance <br /><span className="text-white/20 italic">AI Verticals</span></p>
            </div>
            <p className="text-white/40 max-w-sm text-lg font-medium leading-relaxed">
              Every tool is custom-trained on high-conversion agency frameworks to ensure professional-grade output.
            </p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { icon: Layers, title: 'Scope Builder', desc: 'Generate complex SOWs and technical scopes in under 60 seconds with 95% accuracy.', accent: 'bg-blue-500' },
              { icon: Megaphone, title: 'Pitch Strategist', desc: 'Craft compelling narratives and value propositions that win enterprise clients.', accent: 'bg-purple-500' },
              { icon: BarChart3, title: 'Brief Automator', desc: 'Turn vague client calls into pixel-perfect creative briefs for your production team.', accent: 'bg-pink-500' },
              { icon: Users, title: 'Client Nurture', desc: 'Automated high-touch communication that keeps clients happy and retention high.', accent: 'bg-primary' },
              { icon: Globe, title: 'Ad Scale AI', desc: 'Generate multi-channel ad copy and creative directions based on current virality trends.', accent: 'bg-emerald-500' },
              { icon: Target, title: 'ROI Predictor', desc: 'Forecast campaign impact before spending a single dollar on media buy.', accent: 'bg-amber-500' }
            ].map((item, i) => (
              <GlassCard key={i} className="p-10 border-white/5 hover:border-white/20 transition-all duration-500 group relative">
                ...
                <div className="mt-8 pt-8 border-t border-white/5 opacity-0 group-hover:opacity-100 transition-all flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-primary">
                  Activate Solution <ArrowRight className="w-3 h-3" />
                </div>
              </GlassCard>
            ))}
          </div>
      ...
      <section id="roi" className="py-32 px-6 bg-white/[0.02]">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
           <div className="space-y-8">
              <h2 className="text-xs font-black text-primary uppercase tracking-[0.4em]">The Math of Growth</h2>
              <h3 className="text-4xl md:text-7xl font-black tracking-tighter leading-none uppercase">
                20x More <span className="italic text-white/40">Output</span> <br /> 0x More <span className="italic text-white/40">Hire</span>
              </h3>
              <p className="text-white/40 text-xl font-medium leading-relaxed">
                Antigravity doesn't replace your creatives; it removes the friction that stops them from being creative.
              </p>
              
              <div className="space-y-6">
                 {[
                   { label: 'Time Spent on Scopes', before: '8h', after: '4min', color: 'bg-primary' },
                   { label: 'Brief Preparation', before: '4h', after: '2min', color: 'bg-purple-500' },
                   { label: 'Pitch Deck Logic', before: '12h', after: '15min', color: 'bg-blue-500' }
                 ].map((stat, i) => (
           ...
           <div className="relative">
              ...
              <GlassCard className="p-12 space-y-8 relative z-10 border-white/10 bg-white/[0.03]">
                 <div className="flex items-center gap-4">
                    ...
                    <p className="text-sm font-bold uppercase tracking-widest leading-tight">Increase in <br /><span className="text-primary italic">Billable Efficiency</span></p>
                 </div>
                 <div className="space-y-6">
                    <p className="text-white/30 italic text-lg leading-relaxed">
                      "Since implementing Antigravity, we've scaled from 12 to 45 concurrent projects without adding a single project manager. It's essentially a force-multiplier for my team's intelligence."
                    </p>
                    <div className="flex items-center gap-4">
      ...
      <footer className="py-24 px-6 border-t border-white/5 text-center">
        <div className="max-w-7xl mx-auto space-y-12">
           ...
           <p className="text-white/10 font-medium text-[10px] uppercase tracking-[0.1em]">© 2026 Antigravity Systems. All creative rights reserved.</p>
        </div>
      </footer>
    </div>
  )
