...
async function check() {
  const { data: apps } = await supabase.from('micro_apps').select('id, slug, name_es').eq('slug', 'real-estate-desc')
  console.log('App in DB:', JSON.stringify(apps, null, 2))
  
  if (apps && apps.length > 0) {
    const { data: links } = await supabase.from('plan_apps').select('plan_id').eq('app_id', apps[0].id)
    console.log('Plan links:', JSON.stringify(links, null, 2))
  }
}
