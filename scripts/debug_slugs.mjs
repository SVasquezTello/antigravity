...
async function check() {
  const slugsToCheck = ['reactiva-stay', 'contract-audit-ai', 'course-architect']
  const { data: apps } = await supabase.from('micro_apps').select('slug').in('slug', slugsToCheck)
  console.log('Apps found:', JSON.stringify(apps, null, 2))
}
